module.exports = function (session) {
    ...
    if (this.disableTouch || this.disableTTL) return cb(null, null)
      ...
      this.client.run(`MATCH (s:${this.nodeLabel} {sid: $sid}) SET s.expires = $expires`, { sid: key, expires }).then((res) => {
        cb(null, "OK")
      }).catch(cb)
    ...
    destroy(sid, cb = noop) {
      let key = this.prefix + sid
      this.client.run(`MATCH (s:${this.nodeLabel} {sid: $sid}) WHERE s.sid STARTS WITH $prefix DETACH DELETE s`, { sid: key, prefix: this.prefix }).
      then(() => cb(null, 1))
      .catch(cb)
    }
    clear(cb = noop) {
      this.client.run(`MATCH (s:${this.nodeLabel}) WHERE s.sid STARTS WITH $prefix DETACH DELETE s`, { prefix: this.prefix })
    ...
    ids(cb = noop) {
      this._getAllKeys((err, keys) => {
        if (err) return cb(err)
        cb(null, keys.map(key => key.replace(this.prefix, '')))
      })
    }
    all(cb = noop){
        this.client.run(`MATCH (s:${this.nodeLabel}) WHERE s.sid STARTS WITH $prefix RETURN s`, { prefix: this.prefix }).then(result => {
    ...
    ttl(sid, cb = noop) {
      let key = this.prefix + sid
      this.client.run(`MATCH (s:${this.nodeLabel} {sid: $sid}) RETURN s.expires`, { sid: key })
        .then(result => {
          if (result.records.length === 0) {
            return cb(null, null)
          }
